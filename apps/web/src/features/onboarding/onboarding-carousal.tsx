...
import { CreateCompanyData, createCompanySchema, SUPPORTED_CURRENCIES } from "@repo/shared";
...
export default function OnboardingCarousal() {
    ...
    const { register, reset, getValues, setValue, handleSubmit } = useForm<CreateCompanyData>({
    ...
    return <div>
        ...
        <Card className="w-full min-w-120 max-w-120 h-160">
            ...
            <CardContent className="h-100 flex flex-col gap-4">
                <form id="onboardingForm" onSubmit={handleSubmit(onSubmit, onZodError)}>
                    ...
                    {step == 2 && <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                            ...
                            <Select value={getValues("country")} onValueChange={(e) => {
                                handleCountry(e)
                            }} >
                                ...
                                <SelectContent >
                                    ...
                                    {SUPPORTED_COUNTRIES.map((country) => {
                                        return <SelectItem key={country.name} value={country.code}>{country.name}</SelectItem>
                                    })}
                    ...
                    {step === 3 && <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                            ...
                            <Select value={getValues("defaultCurrency")} onValueChange={(e) => {
                                handleCurrency(e)
                            }} >
                                ...
                                <SelectContent>
                                    ...
                                    {SUPPORTED_CURRENCIES.map((currency) => {
                                        return <SelectItem key={currency.name} value={currency.code}>{currency.name} ({currency.symbol})</SelectItem>
                                    })}
                        ...
                        <div className="grid gap-3">
                            ...
                            <Input  {...(register("invoicePrefix"))} value={getValues("invoicePrefix")} type="text" id="invoicePrefix" />
                        </div>
                        <div className="grid gap-3">
                            <Label htmlFor="taxId">Tax Id <span className="text-muted-foreground">(optional)</span></Label>
                            <Input  {...(register("taxId"))} type="text" id="taxId" placeholder="Gst number, Vat etc" />
                            <p className="text-sm text-muted-foreground">If your business is registered for taxes, enter your registration number (GST, VAT, EIN, etc.) </p>
                        </div>


                        <div className="grid gap-3">
                            <Label htmlFor="defaultInvoiceNote">Default Invoice Note</Label>
                            <Textarea  {...(register("defaultInvoiceNote"))} id="defaultInvoiceNote" className="h-20" placeholder="Type your message here." />
                        </div>
                    ...
                    {step === 4 && <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                            ...
                            <Input  {...(register("website"))} type="text" id="wesbite" placeholder="Enter you website url" />
                        ...
                        <div className="grid gap-3">
                            ...
                            <Input  {...(register("phone"))} type="text" id="phone" placeholder="+91 1234567890" />
                        </div>
            ...
            <CardFooter className="flex flex-col gap-8 items-start ">
                ...
                <div className="flex items-center justify-center gap-2 mb-2">
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            ...
                            className={`h-2 w-8 rounded-full ${step === i ? "bg-secondary" : "bg-muted"
                                }`}
                        />
                    ))}
                </div>
                ...
                <div className="flex justify-between  w-full">
                    ...
                    <Button className=" w-20" onClick={(e) => {
                        e.preventDefault();
                        setStep((prev) => prev - 1);
                    ...
                    {step === 4 ? <Button form="onboardingForm" className="w-20 bg-secondary hover:bg-secondary" type="submit" disabled={step !== 4 ? true : false}>
                        {isPending ? <BasicLoader /> : <p>Finish</p>}
                    </Button> : <Button className="w-20" onClick={(e) => {
                        e.preventDefault();
                        setStep((prev) => prev + 1);
